...
# solve the equations one at a time 
...
parms.highmix <-list(beta=matrix(c(9.089, 2.089, 2.086, 2.037, 2.089, 9.336, 2.086, 2.037, 2.086, 2.086, 9.086, 2.037,2.037, 2.037, 2.037, 9.037),nrow=4,byrow=TRUE),
                     sigma=1/8, gamma=1/5, nu=c(1/(55*365),0,0,0),  mu=c(1/(55*365),0,0,0))
parms.highmix
T0=0                #initial time
S=c()               #initialize S
...
T=c()               #initialize T, a vector to hold times
while(T0<maxTime){  #loop over times
  y=lsoda(y0,c(T0, T0+365),age.model,parms.highmix) #solve diff'l equation for each time
  T=rbind(T, y[2,1])      #store results
  S=rbind(S, y[2,2:5])
  E=rbind(E, y[2,6:9])
  I=rbind(I, y[2,10:13])
...
legend(x='topright',legend=c('<5','6-9','10-19','20+'),  #add legend
       col=c('black','blue','red','green'),lty=1,bty='n')
# What effect does this have on the epidemiological dynamics? 
# With high assortativity, the periodicity is lost. 
# With greater assortativity there is muany more infected,
# which makes since as contact was overall increased. 


# Try to make a function for iterations

run.SI <- function(new.parms){
parms <-list(beta=new.parms, 
                  sigma=1/8, gamma=1/5, nu=c(1/(55*365),0,0,0),  mu=c(1/(55*365),0,0,0))
T0=0                #initial time
S=c()               #initialize S
E=c()               #initialize E
...
run.SI(new.parms = 
         matrix(c(2.089, 2.089, 2.086, 2.037, 2.089, 2.336, 2.086, 2.037, 2.086, 2.086, 2.086, 2.037,2.037, 2.037, 2.037, 20.037),nrow=4,byrow=TRUE))
